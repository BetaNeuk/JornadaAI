...
//A interpolação linear é uma forma de adivinhar um valor entre dois números que você já conhece. Imagine esses dois números como pontos em uma linha. Se você quer encontrar um número entre esses dois pontos, a interpolação linear faz isso usando uma fórmula simples. Basciamente, ela "liga os pontos"de forma reta para pegar o valor intermediário. É muito útil quando precisamos de valores entre dois números conhecidos.
...
class Neuron {
    constructor(inputs) {
        ...
        //O viés (bias) na nossa rede neural é um número que ajuda a rede a aprender. Ele permite que a rede faça ajustes para entender melhor os dados. O bias é importante poque ajuda a rede a aprender padrões nos dados e a tomar decisões melhores. É como um ajuste pequeno que a rede faz para se adaptar ao dados da melhor maneira possível.
    ...
    g(signalList = []) {
        ...
        //Verfica se o neurônio está ativado com base na função tangente
...
class RNA {
    ...
    load(rna) {
        ...
        try {
            ...
            //`catch (e)`vai ser usado para capturar e lidar com erros em JavaScipt, permitindo que você tome decisões específicas quando acontece uma exceção durante a execução do código.
            return;
        }
    }
